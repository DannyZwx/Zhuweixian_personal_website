...
function renderHeader(d){
 const items=d.ui.nav.map(a=>{const isMaking=a.label.includes("花絮"),isEntry=a.label.includes("前页")||a.label.includes("回到前面")||a.label.includes("返回");const cls=[isMaking?"nav-making":"",isEntry?"nav-entry":""].filter(Boolean).join(" ");return `<a class="${cls}" href="${link(a.href)}" data-nav-label="${esc(a.label)}">${esc(a.label)}</a>`}).join("");
